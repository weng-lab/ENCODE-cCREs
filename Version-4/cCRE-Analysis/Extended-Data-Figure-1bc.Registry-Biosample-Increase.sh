...
do
    ...
    python $scriptDir/Toolkit/assign-organ-tissue.py $version.All.txt GRCh38 > tmp.tmp
    awk -F "\t" '{print $3 "\t" $(NF-2)}' tmp.tmp | sort -u > tmp.unique
    python $scriptDir/Toolkit/count.py tmp.unique -1 | awk -F "\t" '{print $0 "\t" "'$version'"}' >> tmp.summary
    awk 'FNR==NR {x[$9];next} ($1 in x)' $dir/Cell-Type-Specific/Core-Collection.txt tmp.unique > tmp.group1
    python $scriptDir/Toolkit/count.py tmp.group1 -1 | awk -F "\t" '{print $0 "\t" "'$version'"}' >> tmp.core
done
