...
awk 'FNR==NR {x[$4];next} !($1 in x)' $genome-ccREs-Tier-1.bed Class-Summary > Nonconcordant-Summary
...
awk 'FNR==NR {x[$1];next} ($4 in x)' tier3-failed-pertinent-data.txt $ccREs > $genome-ccREs-Tier-3.bed
awk 'FNR==NR {x[$1];next} ($4 in x)' no $ccREs >> $genome-ccREs-Tier-3.bed

#cat $genome-ccREs-Tier-1a.bed $genome-ccREs-Tier-1b.bed $genome-ccREs-Tier-2.bed $genome-ccREs-Tier-3.bed \
